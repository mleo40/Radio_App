...
def _registry():
    ...
    return GroupRegistry({"ems": ems})


def test_groups_for_sender():
    reg = _registry()
    assert reg.groups_for("js8call", "KE0XYZ") == ["ems"]
    assert reg.groups_for("meshcore", "a1b2c3d4e5f6") == ["ems"]
    assert reg.groups_for("reticulum", "ff" * 16) == ["ems"]
...
def test_groups_for_message_by_member():
    ...
    assert reg.groups_for_message(msg) == ["ems"]


def test_groups_for_message_by_native_group_tag():
    reg = _registry()
    msg = UnifiedMessage.to_group("W1AW", "EMS", "net starting", transport="js8call")
    assert reg.groups_for_message(msg) == ["ems"]
...
def test_group_claims_its_own_name_without_explicit_tag():
    ...
    reg = GroupRegistry({"ems": Group(name="ems")})
    msg = UnifiedMessage.to_group("W1AW", "EMS", "hi", transport="js8call")
    assert reg.groups_for_message(msg) == ["ems"]
...
def test_groups_for_message_dedupes_member_and_tag():
    reg = _registry()
    # KE0XYZ is a member AND the message carries the ems tag — still one entry.
    msg = UnifiedMessage.to_group("KE0XYZ", "EMS", "hi", transport="js8call")
    assert reg.groups_for_message(msg) == ["ems"]
...
def test_add_and_remove_member_and_save(tmp_path):
    ...
    reg.add_member("ems", "js8call:KE0XYZ")
    reg.add_member("ems", "meshcore:a1b2c3d4e5f6")
    reg.add_tag("ems", "ems")
    reg.save(cfg)
    ...
    assert reloaded.remove_member("ems", "js8call:KE0XYZ")
    reloaded.save(cfg)
    again = GroupRegistry.from_config(Config.load(cfg_path))
    assert again.get("ems").member_specs() == ["meshcore:a1b2c3d4e5f6"]


def test_add_member_is_idempotent():
    reg = GroupRegistry({})
    reg.add_member("ems", "js8call:KE0XYZ")
    reg.add_member("ems", "js8call:ke0xyz")  # same, different case
    assert len(reg.get("ems").members) == 1


def test_remove_group():
    reg = _registry()
    assert reg.remove_group("ems")
    assert reg.get("ems") is None
    assert not reg.remove_group("ems")
